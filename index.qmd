...
affiliations:
  ...
  - id: goldbelt
    name: "Goldbelt Professional Services LLC, United States of America"
  - id: cambridge
...
Mathematical models help interpret data to inform infectious disease policy for both ongoing monitoring and outbreak response.
For evidence to be useful it must be timely, rigorous, and collaborative, yet current modelling approaches struggle to be all three.
...
Other data sources have been shown to provide useful information but remain difficult to integrate due to existing modelling frameworks being custom-built and the complexity of these data sources.
Combining results from multiple models can improve reliability, but without shared components it is unclear whether differences reflect scientific choices or technical details.

Composable models offer a solution.
By allowing parts of models to be shared and reused like building blocks, specialists can contribute their expertise without understanding the whole system.
This makes it easier to combine different data sources reliably whilst remaining flexible enough to adapt to new situations.

We set out what such models need and show they can work by recreating three published analyses using shared components.
This approach could help researchers collaborate more effectively and respond more quickly to future disease outbreaks.
...
For modelling evidence to effectively inform policy it must be timely, explicit about methods and limitations, and as simple as possible whilst remaining rigorous [@whitty2015what].
It should also be collaborative, both across independent modelling efforts [@becker21_modeldissemination] and within models through contributions from researchers with diverse domain expertise [@heesterbeek15_modeling].
During the COVID-19 pandemic, models integrating case counts, prevalence surveys, severity data, and hospital bed occupancy provided evidence that supported policy decisions [@birrell2024; @ons2020cis; @davies21_transmissibility_b117].
Yet in the 2022 mpox outbreak, where similar data sources were available but contact structure and behaviour needed explicit representation, these models lacked the flexibility to be adapted, with new models developed instead that only fit to a single data source due to the need for timely evidence [@Endo2022-ij].
Cross-sectional viral load measurements have been shown to provide useful information on epidemic dynamics [@hay21_estimating], yet this has not been incorporated into most models used to inform policy even where such data was routinely available [@anderson20_reproduction; @ons2020cis].
Whilst multi-model efforts synthesise expertise by combining forecasts or scenario projections from multiple teams (e.g., [@brisson20_impacthpv;@cramer22_unitedstatescovid]) and improve predictive accuracy [@reich22_collaborativehubs;@howerton23_evaluationuscovid], teams rarely collaborate on the components of their models despite shared goals [@becker21_modeldissemination], limiting how quickly evidence can be generated for new contexts.
These examples illustrate that rigorous, timely, and collaborative modelling is achievable.
...
One approach to achieving the flexibility needed for timely and collaborative modelling is to chain multiple models together, fitting separate models sequentially and passing outputs to subsequent stages [@huisman2022;@liu2021npi;@gunther2021nowcasting].
This provides flexibility but at the potential cost of rigour as it makes it difficult to fully propagate uncertainty and can introduce biased estimates [@lison2024].
...
This created a cascade where estimates were used as inputs to subsequent analyses: prevalence estimates became inputs for incidence [@abbott2022estimating] and reproduction number estimates [@mccabe2024inferring], which themselves informed policy analyses [@mccrone2022delta;@nyberg2022omicron;@davies21_transmissibility_b117].
...
This may have impacted the evidence used in policy-relevant decisions about variant transmissibility and severity [@mccrone2022delta;@nyberg2022omicron;@davies21_transmissibility_b117].

Joint modelling, where all data sources and processes are analysed together, avoids these issues and provides a rigorous approach to statistical inference [@gelman2020bayesian; @gelman2025_statisticalworkflow; @lison2024].
However, such models can be difficult to develop, especially within the timelines needed for policy relevant modelling, as they require expertise across multiple domains and careful specification of how different components interact.
Probabilistic programming languages (PPLs) such as Stan or LibBi [@Stan;@murray13_bayesian], and modelling frameworks such as pomp and monty [@king16_statistical;@fitzjohn25_monty], have reduced this development effort by enabling analysts to specify models in high-level code [@vandemeent2018introduction].
However, fragmentation in the field indicates that they are not sufficient to enable the flexibility and cross-domain collaboration needed for policy relevant modelling.
One potential reason is that they do not usually provide straightforward ways to share or reuse model components, meaning that the practical unit of reuse remains the whole model or its codebase, not the underlying epidemiological concepts.
For example, at least seven packages estimate effective reproduction numbers using renewal-based approaches in Stan [@scott2021_epidemia;@epinow2;@epinowcast;@epimap;@epiline;@lison2025episewer;@johnson2025wwinference], yet none share components despite overlapping features and, in some cases, authors.
Reproduction number estimation using wastewater highlights challenges when different domain expertise is required: existing tools were implemented by non-wastewater experts with different levels of focus on sampling details, share no components, and it is not clear which modelling choices matter [@lison2025episewer;@johnson2025wwinference].
To resolve these issues some efforts have aimed for greater generality, for example, the `epinowcast` package aims to provide a modular Bayesian framework to support flexible model specification [@epinowcast].
Yet these efforts often cannot be separated into reusable parts, are difficult for users to contribute to or adapt beyond supported functionality, and remain limited in scope.
...
As an example, advances in `Turing.jl`, a probabilistic programming language built in Julia [@Julia-2017], have introduced a submodel interface that enables composable probabilistic programming, offering the potential for epidemiological model composition [@Nicholson2022-ua; @fjelde2025turin].
In other fields, domain-specific languages (programming languages tailored to specific application domains that use terminology and concepts familiar to domain experts rather than general programming constructs) built in Julia have enabled composable models: `HydroModels.jl` [@jing2025hydromodels] for hydrological modelling and `SpeedyWeather.jl` for weather modelling [@speedyweather2024] are two examples.

In this paper, we first propose a set of requirements for composable infectious disease modelling based on the challenges we have identified, our experience developing infectious disease models during recent outbreaks, and established best practices for Bayesian modelling [@gelman2020bayesian].
We then present a proof of concept to explore how these requirements can be met in practice.
Our proof of concept is composed of a domain-specific language (DSL) and a probabilistic backend, designed to be implemented as an extension of existing or newly developed packages [@juliapackageextensions].
This approach enables building block style model construction through standardised interfaces similar to those used in `SpeedyWeather.jl` [@speedyweather2024].
It also supports composability of a range of model families including ordinary differential equations and mixed equation types with the potential for different computational backends.
We give an overview of our design choices based on our proposed requirements and then provide a detailed explanation of both the DSL and the currently implemented `Turing.jl` backend.
We demonstrate our approach conceptually using models from published epidemiological analyses, and in practice through an autoregressive process model that illustrates composing model components.
We then replicate three published analyses, composing elements of our autoregressive example along with both shared and novel components: a COVID-19 analysis for South Korea using a renewal process [@mishra2020covid]; an extension with reporting delays and day-of-week effects to replicate `EpiNow2`, a real-time nowcasting tool [@epinow2]; and an ordinary differential equation model analysis of influenza outbreak data [@chatzilena2019contemporary].
Finally, we discuss alternative design approaches, evaluate the strengths and limitations of our approach, and identify key areas for future development.
Our proof of concept shows that composable infectious disease modelling can address the tension between statistical rigour and the flexibility needed for collaborative and timely policy-relevant evidence, though work remains to realise this potential.
...
These themes are: uncertainty and inference, model structure, population heterogeneity, accessibility and adoption, interoperability, and modelling workflow.
...
Meeting the requirements in the uncertainty and inference theme (@tbl-requirements) requires reasoning with probabilities, for which probabilistic programming languages (PPLs) are designed[@vandemeent2018introduction].
To meet the requirements in @tbl-requirements, this PPL needs to support automatic differentiation for modern inference (uncertainty and inference theme), enable nesting models within models (model structure theme), and provide access to wide ecosystems for integration with external tools (interoperability theme).
As far as we are aware, only PPLs built in Julia [@Julia-2017] provide the metaprogramming capabilities (writing code that generates or manipulates other code) needed to create domain-specific abstractions that can handle arbitrary stratification, standardised interfaces between components, and programming over the model structure.
Among Julia's PPL options, `Turing.jl` [@fjelde2025turin] best meets our requirements with mature submodel support for nesting models within models, extensive inference algorithm choice, and its implementation as a light abstraction layer (a simplified interface that hides complex implementation details whilst enabling access to underlying functionality) on top of the wider Julia ecosystem.
...
Our approach uses a two-layer architecture with a high-level domain-specific language (DSL) for epidemiological modelling (@sec-dsl) and a low-level implementation using `Turing.jl` (@sec-backend), though importantly we are not locked in to this choice as the DSL is agnostic of the backend used.
This separation enables incremental adoption without rebuilding existing models to use our DSL, with all components remaining functional as standalone tools outside the compositional framework (accessibility and adoption theme, @tbl-requirements).
The DSL layer also provides a clear, concise model specification using epidemiological concepts, enabling domain experts to contribute components encapsulating their specialised knowledge without needing to understand the low-level details of all framework elements.
The backend layer handles the automated bookkeeping of stratification, interface validation, and uncertainty propagation whilst supporting multiple inference approaches and automatic differentiation options, using `Turing.jl` and the wider Julia ecosystems (e.g. [@dalle2025differentiationinterface]).
It also facilitates staged inference approaches such as Markov melding [@goudie2019markov], meaning that components can be fitted separately and combined with proper uncertainty propagation (uncertainty and inference theme, @tbl-requirements).
For further details about the DSL structure and backend implementation, see @sec-materials-methods.

@fig-composable demonstrates how our proposed approach could be used to define models for example epidemiological applications including wastewater surveillance [@lison2025episewer;@johnson2025wwinference], biomarker modelling [@Russell2024-mp;@Hay2022-pp], early outbreak analysis [@scott2021_epidemia;@epinow2;@epinowcast], and incubation period estimation [@Park2024-rl].
Each element of these models would be represented by a component of our proposed DSL: ascertainment, renewal processes, viral kinetics, observation models, and so on.
@fig-composable also highlights the potential for submodel reuse, with the incubation period model appearing in all four applications, the latent infection model in three, and the case observation and viral kinetics models each shared between two applications.
In our proposed framework, each of these submodels would be a self-contained model that could be independently inspected, validated, and reused.

![Demonstration of composability showing how four applications share common components. Four shared submodels are highlighted with purple dashed borders: the Latent infection model (Gaussian Process driving time-varying reproduction number with Generation Time and Renewal, reused across three applications), the Incubation period model (reused across all four applications), the Case observation model (reporting delay, ascertainment, and right truncation, shared between Early Outbreak Analysis and Wastewater Surveillance), and the Viral kinetics model (Random Effect and Piecewise Log-Linear, shared between Biomarker Modelling and Wastewater Surveillance). Applications show full observation pipelines including process models (Shedding Convolution), transformations (Viral Concentration Conversion, Cycle Threshold Conversion), and calibration data inputs. The Incubation period estimation application demonstrates how shared models can be calibrated from independent data sources. Colours indicate component types: statistical processes (orange), process models (blue), latent quantities (light blue), epidemiological parameters (purple), observation modifiers (red), transformations (white), and observation models (green).](figures/fig-composable.png){#fig-composable width="100%"}

To demonstrate our compositional approach in practice, we start with an autoregressive integrated moving average (ARIMA) process, which generalises the autoregressive and random walk latent models commonly used in infectious disease modelling [@mishra2020covid; @epinow2; @chatzilena2019contemporary].
Once implemented, we can reuse it, and its component submodels, in epidemiological applications, as we demonstrate in our case studies.
...
In our DSL, these are implemented as `AR`, `MA`, and `DiffLatentModel` respectively.
There are multiple ways to implement an ARIMA process, but we choose to compose the AR and MA processes into an ARIMA model by modelling the moving average as the residuals of the AR process (@fig-case-studies, ARIMA panel).
Taking this approach would allow us to swap out the MA component for a different process, such as a seasonal component, without needing to modify the AR component.
A full walkthrough using our proposed DSL (@sec-dsl) and `Turing.jl` backend (@sec-backend) is provided in @sec-materials-methods.

![Model structure for the ARIMA with observation error example and three case studies showing component reuse. The ARIMA panel shows how MA(1) feeds into AR(2) to form ARMA (highlighted with purple dashed border as a shared component), which composes with Differencing to create ARIMA(2,1,1), followed by Exponential (Exp) transformation and Poisson observation. Mishra et al. reuses the AR(2) component for modelling the time-varying reproduction number combined with a Renewal process model and Negative Binomial observation. EpiNow2 extends this by reusing the full ARIMA structure (shown as shared component with purple dashed border) with weekly broadcasting, adding Latent Delay for reporting delays and day-of-week ascertainment effects. Chatzilena et al. demonstrates an alternative process model using a Susceptible-Infected-Recovered (SIR) Ordinary Differential Equation (ODE) with Softplus transformation, scaled by population size, and AR(1)-based time-varying ascertainment. Colours indicate component types: statistical models (orange), process models (blue), latent quantities (light blue), epidemiological parameters (purple), observation modifiers (red), broadcasting operations (peach), transformations (white), and observation models (green).](figures/fig-case-studies.png){#fig-case-studies width="100%"}

Whilst our approach is implemented in Julia, we have developed `EpiAwareR` [@EpiAwareR], an R interface to the prototype using `JuliaCall` [@JuliaCall] to demonstrate cross-ecosystem accessibility.
This interface enables R users to access the compositional modelling framework without requiring Julia programming knowledge, specifying models using familiar R syntax whilst gaining access to the full composable component library.
Other similar interfaces could be developed for other programming languages.

## Case studies {#sec-case-studies}

We demonstrate our prototype compositional modelling DSL by recreating three published epidemiological analyses (@fig-case-studies): "On the derivation of the renewal equation from an age-dependent branching process" (referred to as Mishra et al.) [@mishra2020covid], "EpiNow2: Estimate Real-Time Case Counts and Time-Varying Epidemiological Parameters" (referred to as `EpiNow2`)[@abbott-epinow2-wellcomeopenres], and "Contemporary statistical inference for infectious disease models using Stan" (referred to as Chatzilena et al.) [@chatzilena2019contemporary].
@fig-case-studies shows the structure of each model and component reuse across the case studies: Mishra et al. reuses the AR(2) component from the ARIMA example, the `EpiNow2` replication reuses the complete ARIMA model, and Chatzilena et al. demonstrates an alternative Susceptible-Infected-Recovered (SIR) infection process.
...
The first two case studies have also been replicated using the `EpiAwareR` R interface [@EpiAwareR].
...
From our DSL, we use `AR` to generate the latent log $R_t$ trajectory, `Renewal` to compute expected infections via the renewal equation $I_t = R_t \sum_{s} g_s I_{t-s}$, and `NegativeBinomialError` to link expected infections to observed cases with overdispersion.
We assemble these components and use Pathfinder [@zhang2022pathfinder] to initialise NUTS sampling [@hoffman2014nuts].

@fig-mishra shows model checks for each component and posterior analysis.
...
Panels E-F show posterior predictive distributions for daily cases and time-varying $R_t$, recovering the main finding from Mishra et al. that $R_t$ in South Korea peaked at approximately 10 before rapidly dropping below 1 in early March 2020.

![Model components and posterior analysis for Mishra et al. (A) Prior samples from the AR(2) latent process for log $R_t$ over 50 days. (B) Prior samples from the renewal model conditional on a fixed $R_t$ trajectory decreasing from 3 to 0.5. (C) Prior samples from the negative binomial observation model around a latent infection curve. (D) Comparison of the continuous serial interval distribution with its discretised probability mass function. (E) Posterior predictive distribution for daily cases with 50% and 95% credible intervals. (F) Posterior predictive distribution for time-varying $R_t$ on a log scale.](figures/fig-mishra){#fig-mishra width=100%}
...
Our replication reuses the ARIMA(2,1,1) and negative binomial components from @sec-example1, extending them with piecewise constant weekly $R_t$ values, incubation period and reporting delay convolutions [@charniga2024best], and day-of-week reporting effects (@fig-case-studies).
...
From our DSL, we use `broadcast_weekly` to convert the ARIMA(2,1,1) process to piecewise constant weekly $R_t$ values, `LatentDelay` to convolve latent infections with incubation period and reporting delay distributions, and `ascertainment_dayofweek` to multiply expected observations by day-of-week effects via a softmax transformation.

@fig-epinow2  shows model checks for each component and posterior analysis.
...
Panels E-F show posterior predictive distributions, demonstrating that the model explicitly accounts for reporting delays whilst recovering $R_t$ dynamics.

![Model components and posterior analysis for EpiNow2. (A) Prior samples from the piecewise constant by week ARIMA(2,1,1) latent process. (B) Prior samples from the renewal model conditional on a fixed $R_t$ trajectory decreasing from 3 to 0.5. (C) Prior samples from the composite observation model including delays and day-of-week effects around a simulated outbreak curve. (D) Incubation period and reporting delay distributions. (E) Posterior predictive distribution for daily cases. (F) Posterior predictive distribution for time-varying $R_t$.](figures/fig-epinow2){#fig-epinow2 width=100%}

### Contemporary statistical inference for infectious disease models using Stan {#sec-example3}

Chatzilena et al. [@chatzilena2019contemporary] demonstrate Bayesian inference for compartmental disease models using Stan, fitting a Susceptible-Infected-Recovered (SIR) model to single strain influenza outbreak data with both a simple Poisson observation model and a stochastic model with time-varying ascertainment to account for model misspecification and observation error.
...
From our DSL, we compose an SIR compartmental model using `ODEProcess` from the SciML ecosystem [@rackauckas2017differentialequations], a Poisson observation model, and a stochastic observation model using `Ascertainment` for time-varying effects parameterised as an AR(1) process.
We use a softplus transformation to ensure numerical stability when the ODE solver returns small negative values.

@fig-sir shows model checks for each component and posterior analysis.
...
This indicates that the flexible observation model compensates for potential misspecification in the deterministic model as found by Chatzilena et al.

![Model components and posterior analysis for Chatzilena et al. (A) Prior S (top) and I (bottom) compartment trajectories. (B) Prior samples from the stochastic observation model including time-varying ascertainment and Poisson sampling around a simulated outbreak curve. (C) Prior (grey) and posterior $R_0$ distributions comparing deterministic (blue) and stochastic (orange) models; x-axis truncated at 6 to focus on the posterior, cutting the prior tail. (D) Posterior S (top) and I (bottom) compartment trajectories for both deterministic (blue) and stochastic (orange) models. (E) Posterior predictive cases for both models, with median and 50% credible intervals compared to observed data (black points). (F) Posterior $R_t$ over time on a log scale for both models with reference line at $R_t=1$.](figures/fig-sir){#fig-sir width=100%}

# Discussion {#sec-discussion}

In this paper we identified barriers in epidemiological modelling arising from the tension between statistical rigour and the flexibility needed for collaborative and timely policy-relevant evidence, proposed composable infectious disease models as a solution, laid out a set of requirements for such frameworks, and demonstrated through a proof of concept that this approach can address these requirements.
Our proof of concept consists of a domain specific language (DSL) built on top of the `Turing.jl` probabilistic programming language (PPL).
This approach enables building block style probabilistic model construction, analysing all data sources and processes together whilst retaining the modularity of separate model components.
We demonstrated our approach conceptually using models from published epidemiological analyses, and in practice through a worked example implementation of an autoregressive process.
We then replicated three published analyses [@mishra2020covid; @epinow2; @chatzilena2019contemporary], composing elements of our autoregressive process example along with both shared and novel components for each analysis.

<!-- Strengths and weaknesses of our DSL -->

A key strength of our proposed approach, demonstrated through our three case studies, is its modular design, which enables faster, less error prone, model development and component reuse whilst allowing comparison of modelling assumptions without significant reimplementation.
For instance, adding a new data source such as wastewater surveillance to an existing case-based model requires only defining a wastewater model with its own latent and observation process.
This modularity enables interdisciplinary collaboration, as domain experts can contribute specialised components encoding their knowledge without needing to implement other components.
Such a design would also improve both routine forecasting through seasonal refinement of components [@Reich2019-flusight; @Cramer2021-hub-dataset; @cdcrtestimates] and outbreak response through adaptation of models to novel pathogens using the same framework.
For multi-model efforts, common components would enable attribution of different results to modelling assumptions rather than implementation details [@Brockhaus2023-bt].
The component structure of our proposed DSL and validation tools built into it also make our approach well-suited for large language model assisted model construction [@aygun2025ai].
See @sec-approach for additional discussion of the strengths of our proposed approach.
A weakness of our proposed requirements is that they were developed without broader community input, drawing instead on our experience building infectious disease modelling frameworks and applying modelling.
An additional limitation is that our proof of concept does not fully implement all features we think are required, including automated mappings between latent and observed states, partial pooling approaches, and a component library that extends beyond basic epidemiological patterns.
The DSL layer manipulation tooling also cannot coordinate updates of related parameters such as model order and corresponding priors without manual intervention which is key for streamlining model specification.
A final limitation is the learning curve required to adopt this compositional pattern, though this is mitigated as it can be adopted incrementally with researchers embedding elements within their existing models.

<!-- Strengths and weaknesses of Turing.jl -->

Key strengths of building on the `Turing.jl` PPL [@fjelde2025turin] include mature submodel support for nesting models within models, extensive inference algorithm choice, and integration with the wider Julia ecosystem through `Distributions.jl` [@JSSv098i16] and other packages (see @sec-approach for additional detail).
However, building on an evolving probabilistic programming language involves trade-offs.
`Turing.jl` aims to stabilise with its planned 1.0 release, but since developing our DSL, the submodel interface has changed, meaning our current implementation is not compatible with the latest release.
`Turing.jl` also currently has limited handling of numerical instability, so large counts being simulated can cause errors, and vectors that combine missing values with other values require manual handling.
Profiling tools, which are useful for optimising complex models, have historically been difficult to use with `Turing.jl` models because it, as do other probabilistic programming DSLs, performs additional transformations on the original model specification.
However, recent refactoring in `Turing.jl` has aimed to address these limitations.

<!-- Strengths and weaknesses of Julia -->

Key strengths of our approach being implemented using Julia [@Julia-2017] include its ecosystem where domain experts develop specialised tools that interoperate through shared interfaces, access to full programming language features rather than a restricted modelling syntax, multiple dispatch for composition of tools, and the mature SciML ecosystem [@SciML] for differential equations and scientific computing (see @sec-approach for additional detail).
...
Computational overhead from abstraction layers in our DSL is also a concern, though recent performance improvements mean `Turing.jl` is approaching manually optimised models, and new automatic differentiation backends like Enzyme [@moses2020enzyme] and Mooncake [@Tebbutt_Mooncake] could further reduce the computational cost.
Whilst our approach is implemented in Julia, which is not currently common in epidemiology, the R interface [@EpiAwareR] we developed demonstrates that such frameworks can bridge software ecosystems.

<!-- Comparison to literature - Other PPLs -->

The composable modelling concepts we describe are also not limited to `Turing.jl` and could in principle be implemented in PPLs both in Julia and other programming languages.
However, PPLs commonly used in infectious disease modelling do not currently meet our proposed composability requirements (see @sec-requirements).
...
Domain-specific PPLs such as `odin`/`monty` and `greta` trade generality for infectious disease modelling specific features, but none of the available options fully support nesting models within models, and there is limited support for state-of-the-art inference methods [@odin;@fitzjohn25_monty;@greta].
Several other PPLs have some of the features needed to meet our composability requirements.
`Gen.jl` [@cusumanotowner2019gen] supports composition through nested generative function calls and combinators of generative functions, and Gen-based tools have demonstrated useful functionality for epidemiological modelling such as `AutoGP.jl` [@saad2023sequential] for automated Gaussian process kernel composition.
However, Gen focuses on lower-level implementation compared to `Turing.jl`, requiring users to handle more implementation details.
Genify [@tan2021genify] allows for the translation of Julia code into Gen models, potentially easing adoption.
Python-based probabilistic programming languages such as NumPyro [@phan2019composable] and Oryx [@oryx2020] built on JAX [@jax2018github] are also potential options [@pyrenew2024, @coix2024], with possible advantages due to JAX's focus on efficiency and scaling with GPUs.
However, the smaller JAX-specific ecosystem compared to the general Python ecosystem, JAX's optimisation for neural network tasks, and Python PPLs' more programmer-oriented syntax that diverges from mathematical notation may create barriers for epidemiological modellers.
As well as imperative approaches like `Turing.jl`, where models are written as procedural code, declarative graph-based approaches such as `JuliaBUGS.jl` [@sun2024juliabugs] and `RxInfer.jl` [@bagaev2023rxinfer]  that specify models through explicit conditional dependencies between variables are potential alternatives.
...
Some of these ideas have been applied using a rules-based framework to epidemiological modelling in order to enable composition across biological scales [@waites2021rulebased; @waites2022compositional].
...
Our approach currently lacks the formal compositional guarantees of category theory and automated optimisations of symbolic-numeric frameworks.
However, existing AlgebraicJulia tools focus on composing dynamical systems and provide limited support for probabilistic modelling; in contrast, our approach supports the full range of infectious disease modelling methods.
...
Other options for flexible and rigorous infectious disease modelling exist across general statistical tools and domain-specific packages.
At a conceptual level, our approach is similar in spirit to Bayesian Regression Models using Stan (`brms`) [@brms] for Bayesian regression, which provides a composable and user-friendly interface for regression modelling.
Regression frameworks are widely used for applied infectious disease modelling due to their flexibility but can be difficult to adapt to include domain specific elements [@Overton2023-dk].
In comparison, specialised infectious disease tools provide useful, domain-specific, features but are generally designed for a limited number of use cases [@epinow2;@epinowcast;@scott2021_epidemia;@epiline].
...
However, ABM calibration remains challenging, though differentiable ABMs may improve this [@chopra2023differentiable], and there are settings where an ABM approach may not be appropriate.
Our proposed approach could support agent-based backends [@datseris2022agents], and could also support hybrid approaches [@bostanci2025hybrid].
...
This includes integrating our approach with AlgebraicJulia tools for composing dynamical systems, extending category theory based software to support probabilistic models, using symbolic-numeric frameworks for automated optimisation, and exploring alternative PPL backends or building domain-specific components at a lower level of abstraction to enable compatibility with multiple PPLs.
Our proposed requirements could also be used to guide the development of composability features in PPLs.
Methodological advances are also needed for joint estimation of interdependent epidemiological parameters and the integration of different data sources.
...
Better understanding where diversity in modelling assumptions is important in multi-model efforts, and separating such differences from implementation details, also requires further study.
...
Finally, our DSL may enable large language models to reason about model design and propose structural adaptations more easily and with less room for error but further work is needed to assess this.
...
`Turing.jl` could better support composability through improvements to submodel handling, for example by returning random variables from nested submodels without requiring explicit return statements, enabling flexible conditioning on submodels, and simplifying post-processing when variable names change due to the depth of model nesting.
...
Improved tooling for the Bayesian workflow [@gelman2020bayesian; @gelman2025_statisticalworkflow] would strengthen the case for composable modelling in Julia.
...
One sensible development would be to build a `Turing.jl` based composable version of `brms` [@brms], which could provide a non-domain-specific testbed for composable components as well as being directly usable with our domain specific approach.
...
We have shown that composable infectious disease modelling offers a path to maintain statistical rigour whilst enabling the rapid collaborative model development needed to inform policy decisions.
...
Each submodel can itself contain any of these abstract types, enabling flexible composition such as observation models that include latent processes for time-varying ascertainment.

The DSL also supports utilities for model manipulation, inspection, and linking.
Models can be modified in place using tools like `Accessors.jl` [@Accessors.jl], enabling both model iteration and patterns such as partially pooled models that update low-level priors based on grouping structures.
...
The ARIMA panel of @fig-case-studies shows this composition as a directed graph where MA(1) feeds into AR(2) to form the combined process.
...
The `DiffLatentModel`'s `@submodel diff_latent = generate_latent(latent_model.model, n - d)` calls the ARMA model, which in turn calls its composed AR and MA components, and then applies differencing through cumulative summation.
...
Now, using these parameters, we can simulate some data using the generative model by fixing the random variables using the `fix` function. We can then call it, like any normal function, to get simulated observations for `y`.
