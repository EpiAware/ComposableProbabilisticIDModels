...
In this paper we identified barriers in epidemiological modelling arising from the tension between statistical rigour and the flexibility needed for collaborative and timely policy-relevant evidence, proposed compositional infectious disease models as a solution, laid out a set of requirements for such frameworks, and demonstrated through a proof of concept that this approach can address these requirements.
...
We demonstrated our approach conceptually using models from published epidemiological analyses, and in practice through example implementation of an autoregressive process.
...
The explicit component structure of our proposed DSL and validation tools built into it also make our approach well-suited for large language model assisted model construction [@aygun2025ai].
...
Profiling tools, which are useful for optimising complex models, have historically been difficult to use because probabilistic programming abstractions obscure the relationship between source code and runtime behaviour, though recent `DynamicPPL` refactoring has aimed to improve this.
...
Computational overhead from abstraction layers in our DSL is a concern, though recent performance improvements mean `Turing.jl` is approaching manually optimised models, and new automatic differentiation backends like Enzyme [@moses2020enzyme] and Mooncake [@dalle2025differentiationinterface] could further reduce the computational cost.
...
The composable modelling concepts we describe are not limited to `Turing.jl` and could in principle be implemented in PPLs both in Julia and other programming languages.
...
Python-based probabilistic programming languages such as NumPyro [@phan2019composable] and Oryx [@oryx2020] built on JAX [@jax2018github] are also potential options [@pyrenew2024, @coix2024], with potential advantages due to JAX's focus on efficiency and scaling with GPUs.
...
However, existing AlgebraicJulia tools focus on composing dynamical systems and provide limited support for probabilistic modelling; in contrast, our approach supports the full range of infectious disease modelling approaches.
...
Our DSL may enable large language models to reason about model design and propose structural adaptations more easily and with less room for error but further work is also needed to assess this.
...
Improved tooling for the Bayesian workflow [@gelman2020bayesian; @gelman2025_statisticalworkflow] would also strengthen the case for composable modelling in Julia.
...
One sensible development path would be to build a `Turing.jl` based composable version of `brms` [@brms], which could provide a non-domain-specific testbed for composable components as well as being directly usable with our domain specific approach.
