...
Key strengths of building on `Turing.jl` [@fjelde2025turin] include mature submodel support for nesting models within models, extensive inference algorithm choice, and integration with the wider Julia ecosystem through `Distributions.jl` [@JSSv098i16] and other packages (see @sec-approach for additional detail).
...
At a conceptual level, our approach is similar in spirit to Bayesian Regression Models using Stan (`brms`) [@brms] for Bayesian regression, which provides a composable and user-friendly interface.
In comparison, domain-specific infectious disease tools currently provide useful features but are generally designed for a limited number of use cases and do not cover the full range of epidemiological modelling approaches needed in practice [@epinow2;@epinowcast;@scott2021_epidemia;@epiline].
...
Importantly, the composable modelling concepts we describe are not limited to `Turing.jl` and could in principle be implemented on other probabilistic programming languages (PPLs).
However, PPLs commonly used in infectious disease modelling do not currently meet our proposed composability requirements.
