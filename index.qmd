...
Our approach uses a two-layer architecture with a high-level domain-specific language (DSL) for epidemiological modelling (@sec-dsl) and a low-level implementation using `Turing.jl` (@sec-backend).
...
Once implemented, we can reuse it, and its component submodels, in epidemiological applications, such as the case studies below.
