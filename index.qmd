...
Other data sources have been shown to provide useful information but remain difficult to integrate due to the bespoke nature of existing modelling frameworks and the complexity of these data sources.
...
Composable models, where components can be reused across contexts and combined whilst maintaining reliability, could address these issues.
When components have common formats, specialists can contribute their knowledge without understanding entire models.
This allows different data sources to be analysed together reliably, and for specialist knowledge to be included, whilst remaining flexible.

We propose requirements for composable infectious disease modelling based on these challenges.
We demonstrate feasibility through a working example, recreating three published analyses using shared components.
Such systems could provide the timely, rigorous, and collaborative evidence needed to inform policy during both ongoing monitoring and outbreak response.
...
For modelling evidence to inform policy it must be timely, explicit about methods and limitations, and as simple as possible whilst remaining rigorous [@whitty2015what].
...
This provides flexibility at the cost of rigour as it prevents full propagation of uncertainty and can introduce biased estimates [@lison2024].
...
Probabilistic programming languages (PPLs) such as Stan or LibBi [@Stan;@murray13_bayesian], and modelling frameworks such as pomp and monty [@king16_statistical;@fitzjohn25_monty], have reduced this development effort by enabling analysts to specify models in high-level code whilst handling inference [@vandemeent2018introduction].
However, fragmentation in the field indicates that they have not enabled the flexibility and cross-domain collaboration needed for policy relevant modelling.
...
To resolve these issues some efforts have aimed for greater generality, for example, `epinowcast` aims to provide a modular Bayesian framework to support flexible model specification.
...
In other fields, domain-specific languages (programming languages tailored to specific application domains that use terminology and concepts familiar to domain experts rather than general programming constructs) built in Julia have enabled composable models: `HydroModels.jl` [@jing2025hydromodels] for hydrological modelling and `SpeedyWeather.jl` for weather modelling [@speedyweather2024].
For the CIS example, composable submodels would make implicit assumptions explicit and allow publishing intermediate deidentified results from components.
This would make integration and reuse by others less error prone [@goudie2019markov].

In this paper, we first propose a set of requirements for composable infectious disease modelling based on the challenges we have identified, our experience developing infectious disease models during recent outbreaks, and established best practices for Bayesian modelling.
...
This approach enables building block style model construction through standardised interfaces similar to those used in `SpeedyWeather.jl` [@speedyweather2024], supporting composability of a range of model families including ordinary differential equations and mixed equation types with the potential for different computational backends.
...
Through three case studies, we then replicate published analyses, composing elements of our autoregressive example along with both shared and novel components: a COVID-19 analysis for South Korea using a renewal process [@mishra2020covid]; an extension with reporting delays and day-of-week effects to replicate `EpiNow2`, a real-time nowcasting tool [@epinow2]; and an ordinary differential equation model analysis of influenza outbreak data [@chatzilena2019contemporary].
...
The DSL layer provides a clear, concise model specification using epidemiological concepts, enabling domain experts to contribute components encapsulating their specialised knowledge without needing to understand the low-level details of all framework elements.
