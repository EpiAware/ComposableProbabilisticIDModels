...
We reuse the AR(2) model `ar2` defined in the Setup section, but update the first damping coefficient prior ρ₁ to match _Mishra et al_.
...
#|   sampling around a latent infection curve. (C) Prior (grey) and posterior $R_0$
#|   distributions comparing deterministic (blue) and stochastic (orange) models.
#|   (D) Posterior S
...
#|   observed data (black points). (F) Posterior $R_t$ over time on a log scale for
#|   both models with reference line at $R_t=1$."
fig_sir = let
    qs = [0.025, 0.25, 0.5, 0.75, 0.975]
    n_samples = 100
    ts = england_data.ts
