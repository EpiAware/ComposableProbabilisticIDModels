---
title: "Supplementary Information: Composable probabilistic models can lower barriers to rigorous infectious disease modelling"
format:
  html: default
  pdf: default
  ipynb: default
...
The code here demonstrates how our proof of concept compositional modelling DSL recreates and extends existing epidemiological models.
...
We demonstrate how our proof of concept compositional modelling DSL can recreate and extend existing epidemiological models.
...
The EpiAwareR package vignettes provide step-by-step R implementations showing how R users can compose the same models using familiar syntax.

All code and data for reproducing the analyses in this paper are available at: [https://github.com/EpiAware/ComposableProbabilisticIDModels](https://github.com/EpiAware/ComposableProbabilisticIDModels).
...
This data is curated by the `covidregionaldata` R package [@Palmer2021], but we have saved the South Korean data locally.
...
We reuse the AR(2) model `ar2` defined in @sec-setup, but update the first damping coefficient prior ρ₁ to match _Mishra et al_.
They specify ρ₁ ~ N(0.8, 0.05) constrained to [0,1], reflecting strong autocorrelation in the log reproduction number process.
...
The `init_priors=[Normal(0, 0.2), Normal(0, 0.2)]` represent the initial states Z₀ and Z₋₁ of the log reproduction number process.
On the natural scale, this prior for initial R values has approximate mean 1 and standard deviation 0.2.
...
ar_mdl = generate_latent(ar2_mishra, 50)
...
As expected the observed discrete generation interval differs from the underlying continuous distribution due to the double interval censoring process [@charniga2024best].
...
function discretised_gen_plot!(ax, model_data, truth_SI)
    barplot!(ax, model_data.gen_int;
        label="Discretised distribution"
    )
    lines!(ax, truth_SI;
...
@fig-mishra shows that the compositional model defined using our proof of concept recovers the main finding in *Mishra et al*; that the $R_t$ in South Korea peaked at a very high value ($R_t \sim 10$ at peak, @fig-mishra F) before rapidly dropping below 1 in early March 2020, with the model capturing both the epidemic trajectory and day-to-day variation in case counts (@fig-mishra E).

## EpiNow2: Estimate real-time case counts and time-varying epidemiological parameters {#sec-example2}

In this case study, we replicate a common `EpiNow2` configuration using our proof of concept framework.
...
We reuse the ARIMA(2,1,1) model `arima211` defined in @sec-setup but modify it to better reflect outbreak dynamics.
...
@fig-epinow2 D compares the continuous delay distributions with their discretised pmf representations.
...
This code demonstrates component reuse (`negbin` from @sec-example1), layered composition (adding day-of-week effects via `ascertainment_dayofweek`), and sequential composition (adding two delay layers via nested `LatentDelay`).
...
#| echo: false
#| eval: false
# Additional model diagnostics to check for convergence and parameter relationships
using PairPlots, StatsPlots

# Define parameter filtering
param_names = names(epinow2_results.samples)
is_noise_param(name) = occursin("ϵ_t[", string(name))
is_latent_traj(name) = occursin("Z_t[", string(name))
is_dayofweek_param(name) = occursin("DayofWeek", string(name))

# Structural parameters (exclude noise and trajectories, keep first 2 noise as examples, exclude day-of-week)
structural_params = filter(param_names) do name
    if is_noise_param(name)
        return occursin("ϵ_t[1]", string(name)) || occursin("ϵ_t[2]", string(name))
    end
    return !is_latent_traj(name) && !is_dayofweek_param(name)
end

# Pairplot
pairplot(epinow2_results.samples[structural_params])

# Trace plot - same params as pairplot
StatsPlots.plot(epinow2_results.samples[structural_params])
```
...
In _Contemporary statistical inference for infectious disease models using Stan_, *Chatzilena et al.* [@chatzilena2019contemporary] demonstrate Bayesian inference for compartmental disease models using Stan's Ordinary Differential Equation (ODE) solvers.
Their single strain influenza example compares deterministic and stochastic formulations of the SIR model, showing how to perform parameter inference and model comparison for ODEs embedded within a probabilistic programming framework.
We recreate this example using EpiAware, introducing the SIR model as an alternative infection generating process to the renewal model used in the previous case studies.
Unlike Stan, we integrate with existing solver libraries from the SciML ecosystem [@rackauckas2017differentialequations] rather than encoding bespoke solvers, allowing us to leverage state-of-the-art numerical methods and automatic solver selection.
...
r_{SI}(t) &= \beta S(t) I(t), & \text{(Infection rate)} \\
...
\lambda_t &= \text{softplus}(N \cdot I(t))\exp(\kappa_t), & \text{(Expected observations)} \\
...
The population variables $S(t)$, $I(t)$, and $R(t)$ represent proportions of the total population $N$ that are susceptible, infected, and recovered at time $t$.
...
The `SIRParams` struct specifies priors for the transmission rate $\beta$, recovery rate $\gamma$, and initial proportion infected.
Following *Chatzilena et al.*, we use log-normal and gamma priors for $\beta$ and $\gamma$, though we modify their specifications to improve numerical stability and prior interpretability.
*Chatzilena et al.* use $\text{Gamma}(0.004, 0.002)$ (shape, rate parameterisation) for the recovery rate, which has shape < 1 and places most prior mass near zero.
While this acts as a weakly informative prior, it creates numerical difficulties during sampling as $\gamma \to 0$ implies $R_0 \to \infty$.We instead use $\text{Gamma}(8, 0.03125)$ which has a prior mean of 0.25, implying a mean 4-day infectious period appropriate for influenza, while ensuring the prior is bounded away from zero.
*Chatzilena et al.* use $\text{LogNormal}(0, 1)$ for the transmission rate, which combined with their recovery rate prior ensures $R_0 > 1$ but with extreme values.
We instead use $\text{LogNormal}(-0.5, 0.5)$ which, combined with our recovery rate prior, implies an $R_0$ prior with median approximately 3 and 10% prior probability of $R_0 < 1$, providing a more interpretable weakly informative prior for influenza.
We use $\text{Beta}(1, 99)$ for the initial proportion infected to concentrate prior mass near small outbreak initiations.
...
#| echo: false
#| class-output: julia
print(@code_string EpiAware.EpiLatentModels._sir_vf(zeros(3), [1.0, 0.1, 0.0], [0.3, 0.1], 0.0))
```
...
We choose a switching ODE solver which switches between explicit (`Tsit5`) and implicit (`Rosenbrock23`) solvers, helping avoid the ODE solver failing when the sampler tries extreme parameter values.
...
The `ODEProcess` generates infections by solving the ODE system and extracting the infected compartment via `sol2infs`.
EpiAware composes this into a generative model:

```{julia}
#| echo: false
#| class-output: julia
print(@code_string generate_latent_infs(sir_process, 14))
```
...
For the stochastic model, we add time-varying ascertainment using `Ascertainment`, which wraps an observation model with a latent process.
Here we reuse the same `AR` latent model type from earlier case studies, but instead of modelling infection dynamics, we apply it to the observation process.
The AR(1) process $\kappa_t$ operates on the log scale and enters multiplicatively via $\exp(\kappa_t)$.
We specify weakly informative priors: damping near zero (strongly autocorrelated), initial state near zero (minimal baseline adjustment), and small innovation standard deviation.
Unlike earlier uses where we called `generate_latent()` directly, `Ascertainment` takes the latent model as an argument and handles generation internally.
...
#| echo: false
#| class-output: julia
Y_t_demo = [100.0 for _ in 1:14]
print(@code_string generate_observations(ascertainment_model, missing, Y_t_demo))
```
...
# Calculate R0 from SIR samples
sir_R0(samples) = vec(samples[:β] ./ samples[:γ])

# Calculate Rt over time from posterior ODE solutions
function calculate_Rt(sols, samples)
    R0_samples = sir_R0(samples)
    mapreduce(hcat, eachindex(sols)) do i
        sol = sols[i]
...
function sir_trajectory_plot!(ax, sir_process, ts; n_samples=100, alpha=0.1)
    ode_mdl = EpiAware.EpiInfModels._generate_ode_solution(sir_process, 0)
...
function posterior_sir_solutions(sir_process, samples, ts)
    β_samples = vec(samples[:β])
    γ_samples = vec(samples[:γ])
    I₀_samples = vec(samples[:I₀])
    ode_mdl = EpiAware.EpiInfModels._generate_ode_solution(sir_process, 0)
    map(eachindex(β_samples)) do i
        (ode_mdl | (β = β_samples[i], γ = γ_samples[i], I₀ = I₀_samples[i]))()
    end
end
...
#| echo: false
#| output: false
deterministic_R0 = sir_R0(deterministic_sir_results.samples)
stochastic_R0 = sir_R0(stochastic_sir_results.samples)
```
...
fig_sir = let
    ...
    det_posterior_sols = posterior_sir_solutions(
        sir_process, deterministic_sir_results.samples, ts
    )

    # Prior observation model samples with ascertainment (longer time scale for demonstration)
    prior_ts = 1:(7*10)
    latent_infections = [250 * (1 + 0.3 * cospi(2 * t / 30.0)) for t in prior_ts]
    prior_obs_mdl = generate_observations(transform_ascertainment, missing, latent_infections ./ N)
    ...
    ax22 = Axis(fig[2, 2]; ylabel="Cases", xlabel="Days",
        xticks=(ts[1:3:end], t_ticks), xticklabelrotation=π / 4)
    ax23 = Axis(fig[2, 3]; ylabel="Rt", xlabel="Days",
        xticks=(ts[1:3:end], t_ticks), xticklabelrotation=π / 4, yscale=log10)

    # Panel A: Prior S and I trajectories (sample from priors directly)
    ode_mdl_prior = EpiAware.EpiInfModels._generate_ode_solution(sir_process, 0)
    # Sample prior R0 directly from the prior distributions
    prior_R0 = rand(sir_params.infectiousness, 1000) ./
               rand(sir_params.recovery_rate, 1000)
    for _ in 1:n_samples
        sol = ode_mdl_prior()
        if sol.retcode == ReturnCode.Success
    ...
    for col in eachcol(prior_obs_samples)
        scatter!(ax12, prior_ts, col; color=(:grey, 0.2), markersize=3)
    end
    lines!(ax12, prior_ts, softplus.(latent_infections); color=:red, linewidth=3, label="Expected")

    # Panel C: Prior and posterior R0 comparison (filter to 0-6 range)
    R0_filter = x -> filter(v -> 0 < v < 6, x)
    density!(ax13, R0_filter(prior_R0), label="Prior",
        color=(:grey, 0.3), strokecolor=:black, strokewidth=2)
    density!(ax13, R0_filter(deterministic_R0), label="Deterministic",
        color=(:blue, 0.5), strokecolor=:blue, strokewidth=2)
    density!(ax13, R0_filter(stochastic_R0), label="Stochastic",
        color=(:orange, 0.5), strokecolor=:orange, strokewidth=2)
    xlims!(ax13, 0, 6)
    axislegend(ax13, position=:lt)
    ...
    stoch_posterior_sols = posterior_sir_solutions(
        sir_process, stochastic_sir_results.samples, ts
    )

    # Plot individual posterior trajectories
    n_post_samples = min(n_samples, length(det_posterior_sols))
    det_indices = 1:n_post_samples
    stoch_indices = 1:n_post_samples

    # Deterministic model trajectories (plot first)
    for i in det_indices
        sol = det_posterior_sols[i]
        if sol.retcode == ReturnCode.Success
            lines!(ax21_S, sol.t, sol[1, :], color=(:blue, 0.05))  # S compartment
            lines!(ax21_I, sol.t, sol[2, :], color=(:blue, 0.05))  # I compartment
        end
    end

    # Stochastic model trajectories (plot second so they appear on top)
    for i in stoch_indices
        sol = stoch_posterior_sols[i]
        if sol.retcode == ReturnCode.Success
            lines!(ax21_S, sol.t, sol[1, :], color=(:orange, 0.05))  # S compartment
            lines!(ax21_I, sol.t, sol[2, :], color=(:orange, 0.05))  # I compartment
        end
    end

    # Panel E: Combined posterior predictive cases (both models)
    post_predictive_yt!(ax22, england_data, det_posterior_gens; qs, color=:blue)
    post_predictive_yt!(ax22, england_data, stoch_posterior_gens; qs, color=:orange, show_data=false)
    ...
    lines!(ax23, ts, det_Rt_qs[:, 3], color=:blue, linewidth=2)
    band!(ax23, ts, det_Rt_qs[:, 2], det_Rt_qs[:, 4], color=(:blue, 0.3))

    lines!(ax23, ts, stoch_Rt_qs[:, 3], color=:orange, linewidth=2)
    ...
    ax23.ytickformat = values -> [string(round(v, digits=1)) for v in values]
