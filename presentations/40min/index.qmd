...
- PPL must support automatic differentiation,
  nesting models within models, and access to
  wide ecosystems
- Two-layer architecture: high-level DSL for
  specification, low-level PPL for inference
- DSL is agnostic of backend; separation enables
  incremental adoption without rebuilding
  existing models
- Domain experts contribute components using
  epidemiological concepts without needing to
...
- Julia eliminates the two-language problem;
  multiple dispatch enables clean composition;
  SciML ecosystem for ODEs and neural networks
